...
- 검색은 통합 결과에서 Webtoon만 반환하며 첫 페이지만 요청한다.
- 다음 구현 단계는 Next 전체 필터 계약 직접 추출이다.
- 상세·회차·이미지는 필터 검증 후 별도 계약으로 진행한다.
- `0.103`에서 Mangayomi 런타임의 `:not(...)` 선택자 불일치를 제거하고 Latest·검색 표지를 명시적으로 선택한다.

## 18. 2026-07-13 Next Webtoon 필터 구현 체크포인트

- `0.104`에서 Next 전체 필터 모델과 `/api/works` JSON 목록·페이지 처리를 구현했다.
- 자동 계약 검증 이후 Mangayomi 수동 UI 검증 결과에 따라 필터 표시와 상호작용을 조정할 수 있다.
