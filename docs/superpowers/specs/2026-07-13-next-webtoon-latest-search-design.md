...
NTK Webtoon Source ID `260713001`에 Next 계열 Latest 목록과 제목 검색을 추가한다. 기본 도메인은 숫자 설정 `9`로 구성한 `https://sbxh9.com`이며 Mangayomi 선택자 호환성 수정 버전은 `0.103`이다.
...
- 작품 제목: `p.subject`
- 작품 링크: 카드 루트의 `href`
- 표지: `.thumb img`를 DOM 순서로 순회해 `platform-icon` 클래스가 없는 첫 이미지
...
- 다음 묶음 버튼: `nav.pager button[aria-label^="다음"]`
- `nav.pager button[aria-label^="다음"]:not([disabled])`가 하나라도 있으면 `hasNextPage=true`다. pager 밖의 Daum 플랫폼 버튼은 페이지 판정에서 제외한다.
- 마지막 정상 페이지에서 다음 버튼은 모두 `disabled`이며 `hasNextPage=false`다.
- 범위를 넘긴 페이지는 `.ep-empty` 또는 `main.container > div[style*="text-align:center"]`의 정확한 문구 `결과가 없습니다`를 가진 정상 빈 목록으로 처리한다.
...
- 제목: `p.subject`
- 링크: 카드 루트의 `href`
- 표지: `.thumb img.search-thumb-img`의 첫 이미지
...
검색 응답에서 `.ep-empty`가 있으면 정상 빈 결과다. Latest 응답은 `.ep-empty` 또는 `main.container`의 직접 자식 중앙 정렬 div에 정확히 `결과가 없습니다`가 있으면 정상 빈 페이지다. 단, 아무 목록 표식도 빈 표식도 없는 HTML은 구조 오류다.
...
- 버전은 내장 메타데이터와 `index.json` 모두 `0.103`으로 맞춘다.
...
- `.ep-empty`와 중앙 정렬 `결과가 없습니다` 표식의 정상 빈 Latest 페이지
