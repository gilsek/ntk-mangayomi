...
- 기본 파서 계열을 `next`로 변경
- Next 주소 설정은 전체 URL 대신 `sbxh` 뒤의 숫자만 입력받음
...
- 확장 버전을 `0.101`로 올려 클라이언트가 변경을 인식하도록 함
...
Source manifest의 `baseUrl`은 Mangayomi 형식에 맞춰 완전한 기본 주소 `https://sbxh9.com`을 유지한다. 사용자 설정에는 전체 URL 입력란을 노출하지 않고 `Next domain number` 숫자 입력란을 제공한다.

- 기본 입력값: `9`
- 입력값 `9`: `https://sbxh9.com`
- 입력값 `12`: `https://sbxh12.com`
- 허용 형식: 앞뒤 공백을 제거한 뒤 ASCII 숫자 1개 이상
- 설정이 없거나 빈 값: manifest 기본 주소에서 추출한 `9` 사용
- 값이 존재하지만 숫자가 아닌 경우: 요청 전에 명확한 설정 오류

Next URL은 항상 다음 템플릿으로 만든다.

```text
https://sbxh{domainNumber}.com
```

프로토콜, 호스트 접두사나 최상위 도메인이 바뀌는 경우 숫자 설정으로 추측하지 않고 확장 코드를 업데이트한다.

Legacy의 기존 전체 URL 설정과 주소 생성 함수는 삭제하지 않는다. Next 숫자 설정은 별도 preference key와 별도 `getNextBaseUrl` 경로를 사용하여 Legacy 설정과 값이 섞이지 않게 한다.
...
- Next 도메인 숫자의 기본값, 빈 값 fallback, 공백 정리와 URL 조합
- 음수, 소수, 문자와 전체 URL 입력 거부
- Popular 첫 페이지 URL의 `period=week`, `kind=webtoon`
- 사용자 지정 Next 도메인 숫자 적용
...
사용자는 Mangayomi에서 저장소를 갱신한 뒤 NTK Webtoon `0.101`을 적용하여 다음을 확인한다.
...
- `0.101`: Next Webtoon Popular를 처음 적용하는 이번 단계
- `0.2`: Webtoon 전체 기능을 완성하고 Manhwa 개발을 시작하는 시점
- `0.5`: Webtoon, Manhwa, Novel, Anime의 첫 프로토타입이 모두 완성되는 시점

Webtoon 개발 중 `0.101` 이후의 세부 단계는 `0.2`보다 작은 버전을 사용한다.
