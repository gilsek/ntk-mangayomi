...
렌더링된 페이지 이미지는 `.viewer-lazy-img` 요소에 배치되며 URL은 `src`, `currentSrc` 또는 `data-src`에서 얻을 수 있다.
...
1. 실행 즉시 렌더링된 `.viewer-lazy-img`를 확인한다.
2. 유효한 HTTP(S) 이미지 URL을 `currentSrc`, `src`, `data-src` 순으로 수집한다.
3. DOM 순서를 유지하면서 빈 URL과 중복 URL을 제거한다.
4. 이미지가 아직 없으면 짧은 간격으로 다시 확인한다.
...
- 스크립트 실행 직후 DOM을 먼저 확인해 이미 로딩된 경우 추가 대기 없이 반환한다.
